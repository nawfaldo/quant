...
try:
    ...
    from tools.databento_import import (
        DEPTH_SCHEMA,
        SCHEMA_VERSION,
        TICK_SCHEMA,
        TransformState,
...
class SafetyTests(unittest.TestCase):
    ...
    def test_depth_schema_matches_bookmap_shape_and_uses_nanoseconds(self) -> None:
        schema = dict(DEPTH_SCHEMA)
        self.assertEqual(schema["price_level"], "LONG")
        self.assertEqual(schema["size_level"], "LONG")
        self.assertEqual(schema["stream_id"], "SYMBOL")
        self.assertEqual(schema["timestamp"], "TIMESTAMP_NS")
