...
class FeatureAccumulatorTests(unittest.TestCase):
    def test_book_features_and_flow_are_directional(self) -> None:
        ...
        self.assertEqual(row["bid_standing_depth"], 30.0)
        self.assertEqual(row["ask_standing_depth"], 10.0)
    ...
    def test_sweep_fragments_are_reconstructed_as_single_trade(self) -> None:
        features = FeatureAccumulator(0.25)
        features.seed("BID", 100.0, 10.0)
        features.seed("ASK", 100.25, 20.0)
        # 3 unbundled trade fragments within 500us on same side BUY
        t0 = 10_000_000_000
        features.on_event(Event(t0, 1, 1, "T", "BUY", 100.25, 5.0, 100.0, 100.25, ""))
        features.on_event(Event(t0 + 200_000, 1, 2, "T", "BUY", 100.25, 5.0, 100.0, 100.25, ""))
        features.on_event(Event(t0 + 400_000, 1, 3, "T", "BUY", 100.25, 10.0, 100.0, 100.25, ""))
        row = features.finish()

        self.assertIsNotNone(row)
        assert row is not None
        # Total buy volume is 20.0, but trade_count is clustered to 1 sweep
        self.assertEqual(row.values["aggressive_buy_volume"], 20.0)
        self.assertEqual(row.values["trade_count"], 1)
