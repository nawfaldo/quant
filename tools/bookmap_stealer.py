...
subscribed instruments (ES, NQ, etc.) and saves them to the Parquet store via
Line Protocol.
...
# Bookmap copies external Python addons to C:\Bookmap\Python\tmp before running
# them, without copying sibling modules. Prefer the script directory for normal
# launches, then fall back to the repository directory for Bookmap launches.
SCRIPT_DIR = Path(__file__).resolve().parent
module_dirs = [SCRIPT_DIR]
configured_module_dir = os.getenv("QUANT_DATA_MANIPULATION")
if configured_module_dir:
    module_dirs.append(Path(configured_module_dir))
module_dirs.append(Path.home() / "Quant" / "tools")
for module_dir in module_dirs:
    if (module_dir / "build_nq_l2_features_1s.py").is_file():
...
# Where rows land, for the startup banner
#: Kept only so the startup banner can name where rows land.
STORE_LABEL = "data/parquet"
...
# ============================================================================
# STORE WRITER METHODS
# ============================================================================

def get_sender() -> Sender:
    """Get (or create) the Parquet writer.

    `dedup=True`, unlike the bulk importers: this is a live collector, it
    re-sends the second it was in the middle of whenever it restarts, and
    those really are the same row.
    """
    global sender, pending_rows
    if sender is None:
        print(f"[COLLECTOR] Opening writer: {STORE_LABEL}", flush=True)
        s = Sender()
        s.__enter__()
        sender = s
        pending_rows = 0
        print("[COLLECTOR] Writer ready.", flush=True)
    return sender
...
def spool_rows(rows: list[Row], reason: str) -> None:
    ...
    or replayed, avoiding accidental duplication in the store after an uncertain
...
def send_row_safe(
    ...
    """Quickly hand an immutable row to the single store writer thread."""
...
def write_row(row: Row) -> bool:
    """Buffer one queued row in the store writer."""
    global pending_rows
...
def writer_loop() -> None:
    """Own store writes, periodic flushing, and collector health reporting."""
    ...
    def flush_unflushed() -> None:
        ...
        with sender_lock:
            ...
            try:
                ...
                # A failed flush has an uncertain outcome -- a shard rename
                # either happened or did not, and this side cannot tell which.
                # Preserve the complete batch for explicit recovery rather than
                # assume any buffered row landed.
                spool_rows(unflushed_rows, "store_flush_failed")
                unflushed_rows.clear()
                clear_sender()
    ...
    while running or not row_queue.empty():
        ...
        try:
            ...
            if write_row(row):
                ...
                spool_rows(unflushed_rows + [row], "store_write_failed")
                unflushed_rows.clear()
        except queue.Empty:
            pass
        finally:
...
# ============================================================================
# WEBSOCKET LIVE PUSH (sub-100ms path to the browser, bypasses the store)
# ============================================================================
# The store path persists every tick but has flush + shard-rewrite latency that
...
# arrives in on_trade. Browsers receive ticks in ~1-10ms; the store stays the
...
def start_ws_server() -> None:
    ...
    if not _WS_AVAILABLE:
        print("[WS] 'websockets' not installed — live push DISABLED. "
              "Run: pip install websockets  (chart falls back to store polling)", flush=True)
        return
...
def on_trade(
    ...
    try:
        ...
        # Persistence path: buffered write to the store (historical backfill).
...
if __name__ == "__main__":
    print("=" * 60, flush=True)
    print("[COLLECTOR] Bookmap Trades + L2 DOM Collector - Optimized", flush=True)
    print(f"[COLLECTOR] Store: {STORE_LABEL}", flush=True)
    ...
    # Warm up when available, but still start the addon if the store is not writable:
    # failed rows will be retained in the append-only local spool.
    try:
        get_sender()
    except Exception as exc:
        print(
            f"[COLLECTOR] store unavailable at startup; rows will spool: {exc}",
            flush=True,
        )
        clear_sender()
    ...
    writer_thread = threading.Thread(
        target=writer_loop, daemon=True, name="store-writer"
    )
    writer_thread.start()
    feature_running = True
    ...
    # Stop feature production before allowing the store writer to drain.
