"""One intraday family study for any CFD symbol, priced at Exness or FundedNext.

WHICH BROKER. `CFD_BROKER=exness` (the default, and every sealed result) or
`CFD_BROKER=fundednext`, or `--broker` on the command line. The broker decides
the spec snapshot (contract, swaps, spread), the commission, the broker's own
minute table the fills are priced from (`exness_*_1m` / `fundednext_*_1m`), the
symbol names, and the prefix of every result file (`exness_families_*` /
`fundednext_families_*`). The rules, the vendor bars they decide on and the
gates are the same for both. Everything below was written for Exness Pro and
holds for FundedNext except where `BROKERS` says otherwise.
...
and everything about it is looked up:
    ...
    python -m sandbox.research.cfd_families select --symbols ukoil,aus200
    python -m sandbox.research.cfd_families select --symbols all
...
this module scores instead is the path `fill_models.exness` reads out of the
...
already breached. `fill_models.exness cells` splits entry from exit if a
...
#:
#: COMMISSION, per asset class (`CLASS`), charged ONCE per trade -- both brokers
#: bill at the open and nothing at the close. `("usd_per_lot", x)` is x USD per
#: lot; `("notional_pct", x)` is x percent of lots * contract * open price.
#: Exness Pro: none, measured (see THE COST MODEL below). FundedNext: from its
#: help centre, read 2026-09-24 -- forex $7 per lot, commodities 0.0016%,
#: crypto 0.04%, stocks 0.004%, indices free, "charged only at trade opening".
#: FundedNext names metals as commodities; the oils are ASSUMED to be too, which
#: the help centre does not spell out.
#:
#: `tick_clock` is what a terminal tick's `time` means: `utc` on Exness,
#: FundedNext server time (New York + 7h) on FundedNext.
BROKERS = {
    "exness": {
    ...
    "fundednext": {
        "account": "FundedNext 35018676 (FundedNext-Server 3)",
        "terminal": r"C:\Program Files\FundedNext MT5 Terminal\terminal64.exe",
        "spec_path": os.path.join(RESULTS, "fundednext_specs.json"),
        "table_prefix": "fundednext_",
        "tick_clock": "ny_plus_7",
        "commission": {"forex": ("usd_per_lot", 7.0),
                       "commodity": ("notional_pct", 0.0016),
                       "crypto": ("notional_pct", 0.04),
                       "stock": ("notional_pct", 0.004)},
        "cost_model": "spread + commission once at the open: forex $7/lot, "
                      "commodities 0.0016%, crypto 0.04%, stocks 0.004% of "
                      "notional, indices 0 (FundedNext help centre, 2026-09-24)",
    },
}

#: Which broker this process prices. Read from the environment first so a
#: spawned worker inherits its parent's choice.
BROKER = os.environ.get("CFD_BROKER") or _argv_broker() or "exness"
if BROKER not in BROKERS:
    raise SystemExit(f"unknown broker {BROKER!r}; known: {sorted(BROKERS)}")
PROFILE = BROKERS[BROKER]
SPEC_PATH = PROFILE["spec_path"]
RESULT_PREFIX = f"{BROKER}_families"
...
#:
#: RESTORED FOR THE `fastbar` WAVE. That measurement was of day-scaled rules
#: on a finer bar; `fastbar` is rules DEFINED in bars -- RSI(14), EMA 9/21 --
#: which only mean what a chart trader means at 5m and 15m. The old finding
#: still stands for every other group: run them at 5m and expect the decay.
BAR_CHOICES = (5, 15, 30, 60, 120, 240, 1440)
...
#: FundedNext's names where they differ from `symbol.upper()`. Probed 2026-09-24
#: from `symbols_get`: it quotes no crypto, no XAL/XNI and no gold crosses, and
#: its index and oil names follow neither the repository nor Exness.
FUNDEDNEXT_ALIAS = {"ustec": "NDX100", "nq": "NDX100", "es": "SPX500",
                    "us500": "SPX500", "de40": "GER30", "fr40": "FRA40",
                    "stoxx50": "EUSTX50", "ukoil": "UKOUSD", "usoil": "USOUSD"}
...
def _mt5():
    ...
    # THE BROKER'S OWN TERMINAL, BY PATH. With two terminals open a bare
    # `initialize()` attaches to whichever one MetaTrader picks, so a FundedNext
    # spec read could silently come back from Exness. A machine without the
    # installed path falls back to the bare call.
    terminal = PROFILE["terminal"]
    started = (mt5.initialize(path=terminal) if os.path.exists(terminal)
               else mt5.initialize())
    if not started:
        raise SystemExit(f"cannot reach the {BROKER} MT5 terminal: "
                         f"{mt5.last_error()}")
    return mt5
...
def broker_symbol(symbol):
    if BROKER == "fundednext":
        return FUNDEDNEXT_ALIAS.get(symbol, symbol.upper())
...
def _read_fx(mt5, specs):
    ...
    for currency in sorted(c for c in wanted if c and c != "USD"):
        for name, invert in ((f"{currency}USD", False), (f"USD{currency}", True)):
            ...
            tick = mt5.symbol_info_tick(name)
            # A symbol selected a moment ago has no tick yet; FundedNext's
            # USDHKD came back empty on the first read and HKD went unpriced.
            for _ in range(3):
...
def tick_spreads(symbols, days=7.0, path=SPEC_PATH, quiet=False):
    ...
    for symbol in symbols:
        ...
        for tick in ticks:
            ...
            every.append(spread)
            if PROFILE["tick_clock"] == "ny_plus_7":
                # Server time sits seven hours ahead of New York all year
                # (`tools/exness_import_1m.py`, FUNDEDNEXT CLOCK), so the New
                # York minute is plain arithmetic, no zone lookup.
                minute = ((int(tick["time"]) - 7 * 3600) % 86400 // 60
                          + shift) % 1440
            else:
                moment = datetime.fromtimestamp(int(tick["time"]),
                                                tz=timezone.utc)
...
#: THE MULTI-DAY LIVE MODEL. Both off for every sealed result; set from the
#: environment so spawned workers see them, and recorded in the seal.
#:
#: `FULL_DAY` keeps every bar the market prints instead of the session window.
#: A `multiday` family still ENTERS only in session, where the spread is the
#: one the study priced, but it is held and WATCHED on every bar -- the account
#: carries no broker-side stop, so a position left overnight is only protected
#: if the runtime reads the overnight bars. Without this the stop is inert from
#: the close to the next open ([[session-filtered-bars-make-overnight-stops-inert]]).
#: Porting a `multiday` sleeve therefore means the runtime and the feeds daemon
#: must watch these markets around the clock, not just in session.
FULL_DAY = os.environ.get("EXNESS_FULL_DAY") == "1"
#: The account's swap-free status. Unknown on 2026-09-23: the one position that
#: crossed a 17:00 New York rollover (a USDJPY short, spec -13.7 points) was
...
#: WEEKEND ALONE -- the hours a crypto sleeve can trade while every FX and index
#: sleeve is shut, and the only hours a PC that is off the rest of the week
#: would need to be on. Only entries are gated: a position still exits by its
#: own rules, and under `SESSION_ONLY` that is inside the same session anyway.
#: The day is the stamp's own calendar day, which for every unshifted market is
#: the New York date. A weekend run seals to its own file (`output_path`), so it
#: can never overwrite the seven-day study.
_WEEKDAYS = {"mon": 0, "tue": 1, "wed": 2, "thu": 3, "fri": 4, "sat": 5, "sun": 6}


def _entry_days(text):
    if not text:
        return None
    days = frozenset(_WEEKDAYS[part.strip().lower()[:3]]
                     for part in text.split(",") if part.strip())
    return days or None


ENTRY_DAYS = _entry_days(os.environ.get("EXNESS_ENTRY_DAYS"))


def entry_days_tag():
    """`sat+sun`, say, for a restricted run; `None` for every day."""
...
def resolve(symbol, allow_stale=False, specs=None):
    ...
    # late market order (`fill_maps`). Without it there is nothing to price
...
def periods(symbol, bar=None):
    ...
    per_session = (1 if is_daily(bar)
                   else 1440 // (bar or BAR_MINUTES) if FULL_DAY
...
def commission_rule(symbol):
    """`(kind, value)` for `symbol` at this broker, or None when it is free."""
    return PROFILE["commission"].get(CLASS.get(symbol, "other"))


def commission_price(symbol, entry):
    """The broker's commission for one trade, in PRICE units per lot.

    Charged once -- both brokers bill at the open only -- and in price units so
    it rides the same `lots * multiplier` the gross move does. A dollar amount
    per lot divides by `multiplier`, which is already USD per unit of price per
    lot; a percentage of notional is that percentage of the entry price.
    Exness Pro has no rule and returns 0, so its results are unchanged.
    """
    rule = commission_rule(symbol)
    if not rule:
        return 0.0
    kind, value = rule
    if kind == "usd_per_lot":
        multiplier = INSTRUMENTS[symbol]["multiplier"]
        return value / multiplier if multiplier else 0.0
    if kind == "notional_pct":
        return entry * value / 100.0
    raise SystemExit(f"{symbol}: unknown commission kind {kind!r}")


def commission_bp(symbol, price=None):
    """`commission_price` in basis points of notional, at `price`.

    For REPORTING only -- a dollar-per-lot charge is a different number of basis
    points at every price, so the engine charges `commission_price` and this is
    the figure at the snapshot's reference price.
    """
    price = price or INSTRUMENTS[symbol].get("reference_price") or 0.0
    return 1e4 * commission_price(symbol, price) / price if price else 0.0


def cost_bp(symbol, spread_bp=None):
    """Everything charged, once at entry, in basis points of notional.

    The quoted spread, a slippage allowance and the broker's commission
    (`commission_bp`, zero on Exness Pro -- measured, see the module docstring).
    Basis points because that is the only unit that compares a $2.83 gas
    contract with a $30,000 index without ranking them by price level.
    """
    spread = (INSTRUMENTS[symbol]["spread_bp"] if spread_bp is None else spread_bp)
    return (spread or 0.0) + SLIPPAGE_BP + commission_bp(symbol)


def cost_price(symbol, entry, spread_bp=None):
    """Everything charged at entry, in price units at this entry price."""
    spread = (INSTRUMENTS[symbol]["spread_bp"] if spread_bp is None else spread_bp)
    return (entry * ((spread or 0.0) + SLIPPAGE_BP) / 1e4
            + commission_price(symbol, entry))
...
#: New York wall-clock minute of the daily swap rollover. Exness, like most
#: brokers, rolls at 17:00 New York (21:00 or 22:00 GMT with DST). ASSUMED, not
#: yet measured on this account: no position here has been held through enough
#: rollovers to read the time off the deal history.
ROLLOVER_MINUTE = 17 * 60


def rollover_charges(symbol, entry_ts, exit_ts):
    """Swap charges a position incurs between two bar stamps, in nights.

    THE BROKER'S RULE, WHERE `financing_nights` COUNTS CALENDAR DATES. Over a
    whole week the two agree (7), but not per trade: a Thursday-to-Monday FX
    hold pays 2 rollovers, not 4 calendar nights, and a Tuesday-to-Thursday one
    pays 1 + 3 = 4, not 2. A `multiday` family's entries cluster by weekday
    often enough for that to matter, so it is billed this way; every older
    family keeps `financing_nights` and its sealed numbers.

    One charge at each 17:00 New York rollover the position is open through,
    tripled on `swap_rollover3days` (MT5 numbering, 0 = Sunday) and zero on a
    Saturday or Sunday rollover -- except on a 365-day market, where the broker
    charges every night and there is no triple. Read on the REAL clock: the
    shifted Asian markets have their stamps moved back first
    ([[shifted-markets-need-the-shifted-clock-everywhere]]).
    """
    spec = INSTRUMENTS[symbol]
    shift = (spec.get("shift_hours") or 0) * 3_600
    start, end = entry_ts - shift, exit_ts - shift
    offset = ROLLOVER_MINUTE * 60
    first = (start - offset) // 86_400 + 1
    last = (end - offset) // 86_400
    if last < first:
        return 0
    if spec.get("calendar", 252.0) >= 365.0:
        return last - first + 1
    triple = spec.get("swap_rollover3days")
    triple = 2 if triple is None else (int(triple) - 1) % 7   # Python weekday
    total = 0
    for day in range(first, last + 1):
        weekday = (day + 3) % 7                               # 1970-01-01 = Thu
        if weekday >= 5:
            continue
        total += 3 if weekday == triple else 1
    return total
...
def context(symbol, phase, bar=None, only=None, full_bars=None):
    ...
    bars = (full if daily or FULL_DAY
            else [b for b in full if in_session(symbol, b[TS])])
    ...
    # A RUN OF ONLY THE BAR-COUNT WAVES READS ALMOST NONE OF THE ABOVE. Their
    # signals read `ctx["fb"]` and nothing else, and `backtest` reads the short
    # list in `BAR_WAVE_KEEPS`; the other forty-odd arrays were 0.5-1 GB per
    # worker at 5m, and memory is what caps the pool. Dropped only when EVERY
    # requested family is `fb_`/`sb_`, so no other run is touched.
    if only and all(name.startswith(("fb_", "sb_")) for name in only):
        for key in [k for k in ctx if k not in BAR_WAVE_KEEPS]:
            del ctx[key]
        gc.collect()
    return bars, compact(ctx)


#: What `backtest`, `quantity` and the bar-count signals read from a context.
BAR_WAVE_KEEPS = frozenset((
    "atr", "calm", "cfg", "daily", "day", "minute", "ema", "overnight",
    "periods", "risk", "session_index", "session_last", "symbol", "ts",
    "volatility", "vol_target", "bar_minutes", "fb", "spread_rank"))
...
def extra_context(symbol, phase, bars, ctx, p, daily, bar, blocks=None):
    ...
    if "spread" in blocks:
        # THE COST MODEL, READ AS A SERIES. `install_fills` puts the
...
# CANNOT SEE. Since `install_fills`, the spread the account crossed in
...
#: THE NINTH WAVE: textbook indicators at textbook settings, counted in BARS.
#: Defined in `exness_fastbar`, which says why; registered here so the engine,
#: the gates and the seal treat them like every other family. Each reads only
#: its own `fb:<name>` block, so a `--groups fastbar` run builds the series its
#: cells can reach and nothing else.
for _name, (_signal, _axes, _keys) in fastbar.FAMILIES.items():
    FAMILIES[_name] = Family(
        _signal,
        (lambda axes: lambda p, g: {
            **axes, "last_entry_minute": g["last"], **g["fastbar"]})(_axes),
        scope="intraday", reads=(f"fb:{_name}",))

#: THE TENTH WAVE: the same fifty-three constructs, HELD FOR DAYS. Registered
#: as `sb_<name>` with the `multiday` regime, which holds through every close,
#: bills swap on the broker's rollover schedule and marks drawdown to market.
#: Meant for `--bar-minutes 60, 240, 1440` with `EXNESS_FULL_DAY=1`, where a
#: bar-count setting is a swing setting -- RSI(14) on 4-hour bars is 56 hours.
for _name, (_signal, _axes, _keys) in fastbar.FAMILIES.items():
    FAMILIES["sb_" + _name[3:]] = Family(
        _signal,
        (lambda axes: lambda p, g: {
            # No entry cutoff: a held position has no close to be late for.
...
GROUPS = {
    ...
    #: The ninth wave -- see `exness_fastbar`. Short bar-count settings, meant
    #: for `--bar-minutes 5` and `15`.
...
def axes(symbol, bar=None, only=None):
    ...
    # THE FASTBAR GRID. Same shape as `common`, with exits a bar-scale rule can
    # reach: a tenth of a day's range as the tightest stop, clock exits at 6
    # and 24 bars, and a trail a quarter of a day's range behind -- `trail_1.5`
    # trails one and a half DAYS and never fires inside a 5-minute trade.
    fast = {"exit_mode": fastbar.FASTBAR_EXIT_MODES,
            "stop_day": fastbar.FASTBAR_STOP_DAY,
            "trend": fastbar.FASTBAR_TREND, "vol_mode": fastbar.FASTBAR_VOL}
    # THE SWINGBAR GRID. Exits a multi-day hold can reach: 2R and 3R targets,
    # 2/5/10-day clocks, and trails a half and a whole day's range behind.
    # Stops start at half a day's range, since anything tighter is noise over
    # a hold of days.
    held = {"exit_mode": fastbar.SWINGBAR_EXIT_MODES,
            "stop_day": fastbar.SWINGBAR_STOP_DAY,
            "trend": ("none", "ema_50d"), "vol_mode": ("none", "calm")}
...
#: one constant. Filled by `install_fills` from the broker's own minute
...
#: skipped or zeroed. Filled by `install_fills`.
ENTRY_PRICE = {}
EXIT_PRICE = {}
...
#: that the queue is free. `fill_models.exness.BRIDGE_QUEUE_SECONDS` is the
...
#: STRICT LIVE MODEL. Off, a bar the broker's minute table does not reach is
#: filled at the vendor open with the constant spread -- the idealised fill --
#: which is 15% of ETHUSD's in-sample bars at 5m. On, a signal whose fill bar
#: has no live entry price is not taken at all, so every trade the run books
#: was priced the way the account fills. It is not counted as a signal either,
#: so `fill_rate` still measures lot granularity alone. Off is every sealed
#: result; set from the environment so spawned workers see it.
FILLS_ONLY = os.environ.get("EXNESS_FILLS_ONLY") == "1"

#: Which broker's fill path prices the run -- a module name under
#: `sandbox/research/fill_models/`. Set from the environment so spawned workers
#: see it; `exness` is every sealed result.
FILL_MODEL = os.environ.get("EXNESS_FILL_MODEL", BROKER)

#: A `multiday` position's open profit counts toward the PEAK its drawdown is
#: measured from, not only toward the trough.
#:
#: Off, the peak moved only when a trade CLOSED, so a hold that ran up and gave
#: most of it back before its trail hit booked no drawdown for the give-back:
#: marked 1,100 then closed at 1,010 read as 0% where the account was 8.2% off
#: its high. That is the one number the `MAX_DD` gate reads, and it read low on
#: exactly the trailing exits the swing grid is built on. Measured 2026-09-25
#: over the 412 sealed swing winners (jp225 and hk50 aside): in-sample drawdown
#: +1.3 points on average and up to +5.7, and 49 of them fail the 20% gate once
#: it is measured this way.
#:
#: ON BY DEFAULT because it is a measurement fix, not a design choice.
#: `EXNESS_MTM_PEAK=0` reproduces every swing seal written before it, which is
#: the only thing the old behaviour is still for. Set from the environment so
#: spawned workers see it, and recorded in the seal.
MTM_PEAK = os.environ.get("EXNESS_MTM_PEAK", "1") == "1"


def fill_model():
    """The `fill_models` module `FILL_MODEL` names."""
    import importlib  # noqa: PLC0415

    return importlib.import_module(f"sandbox.research.fill_models.{FILL_MODEL}")


def broker_minute_table(symbol):
    """The store table holding the broker's own one-minute bars for `symbol`."""
    return f"{PROFILE['table_prefix']}{broker_symbol(symbol).lower()}_1m"
...
def fill_maps(symbol, bar=None):
    ...
    Built with `fill_models.exness`'s readers off `exness_<broker>_1m`, which
    ...
    return data._cached(f"fills_{FILL_MODEL}_{symbol}_{label_bar(bar)}", key,
                        build)


def install_fills(symbol, bar=None, quiet=False):
    ...
    """
    spreads, entries, exits = fill_maps(symbol, bar)
    ...
    if not quiet:
        ...
        print(f"{symbol} {label_bar(bar)}: {FILL_MODEL} fills from "
...
#: Signal settings whose firing bars a worker keeps. Cells arrive grouped by
#: signal setting (the exit/stop/filter axes vary fastest), so a small cache
#: is hit ~60 times per entry; the bound keeps memory flat across a family.
FIRE_CACHE_SIZE = 64
_FIRE_EXIT_AXES = frozenset(("exit_mode", "stop_day", "trend", "vol_mode",
                             "stop_multiple"))
...
THREE HOLDING REGIMES. A `session` family is flattened at the close, as every
    ...
    `install_fills` filled for this symbol, so the spread charged is the
    ...
    # `multiday` IS EXEMPT FROM SESSION_ONLY BY DECLARATION. That flag took the
    # holding away from every older swing family; this regime exists to hold,
    # and is priced with the rollover schedule and marked to market below.
    multiday = hold == "multiday"
    swing = swing or multiday
    ...
    # zeroed. `ENTRY_PRICE` is what `install_fills` puts there, so every
    ...
    # THE FLAT-ACCOUNT SKIP, for the bar-count waves only. While nothing is held
    # and nothing is pending, the only thing a bar can do is fire an entry, and
    # an `fb_`/`sb_` signal is a pure function of the bar index and the cell's
    # SIGNAL axes -- it reads `ctx["fb"]` and nothing else, keeps no state, and
    # ignores the exit, stop, trend and volatility axes. So the bars where it
    # fires are computed once per signal setting and shared by the ~60 cells
    # that differ only in how they exit, and the loop jumps from one firing
    ...
    while True:
        index += 1
        if fires is not None and position is None and pending is None:
            at = bisect.bisect_left(fires, index)
            while at < len(fires) and day_of[fires[at]] == traded_day:
                at += 1
            if at >= len(fires):
                break
            index = fires[at]
        ...
        # A `multiday` family watching the whole day may also DECIDE on any bar
        # the market prints: at 240m only one bar a day sits inside a US cash
        # session, and its fill bar is always past the bell.
        tradeable = (daily or (multiday and FULL_DAY)
        ...
        if position is not None:
            ...
            if (price is None and position["trail"] is not None
                ...
                nights = (rollover_charges(symbol, position["ts"], ts) if multiday
                          else financing_nights(position["ts"], ts))
                carried = (0.0 if multiday and SWAP_FREE
                           else nights * financing[0 if side == 1 else 1])
        ...
        # MARKED TO MARKET, for the regime that holds for days. Every other
        # family books drawdown on closed trades alone, which is close enough
        # for a position that lives inside one session and badly wrong for one
        # that sits through a week ([[engine-drawdown-is-mark-to-market]]). The
        # open position is marked at the bar's close net of its entry cost.
        if multiday and position is not None:
            if "mtm_cost" not in position:
                position["mtm_cost"] = cost_price(symbol, position["entry"],
                                                  position["spread_bp"])
            open_points = (position["side"] * (fbar[C] - position["entry"])
                           - position["mtm_cost"])
            marked = equity + open_points * position["lots"] * money_per_point
            if MTM_PEAK:
                peak = max(peak, marked)
            if peak > 0:
                maximum_dd = max(maximum_dd, (peak - marked) / peak)
        ...
        if (position is None and pending is None and traded_day != day
            ...
            if (side is not None and atr and present(atr)
                ...
                if accepted and FILLS_ONLY and by_entry is not None:
                    ready = index + 1 + lag
                    accepted = (ready < len(bars)
                                and bars[ready][TS] in by_entry)
                if accepted:
...
#: How `choose` ranks the cells that clear every gate.
#:
#:   tstat    the per-trade t-statistic of the edge over drift
#:            (`edge_vs_drift_t_stat`), `quality` breaking ties
#:   growth   `quality` alone -- compounded growth -- which is how every seal
#:            before 2026-09-25 was chosen
#:
#: GROWTH PICKS THE LUCKIEST CELL, AND THE LUCKIEST CELL IS THE NOISIEST ONE.
#: `selection_study` scored every cell of 53 swing families on seven markets
#: at 240m, both windows, and a winner's in-sample growth ranked its holdout
#: return at Spearman -0.12 -- the more it made, the less it kept. Ranked on
#: the t-statistic instead, the SAME 185 families did +1.7 points better on the
#: holdout (t 2.47) at 15.1% mean drawdown against 18.3%, better on five of the
#: seven markets. The two it lost on are the two with an edge to lose (ethusd
#: -4.5 over 7 families, de40 -0.7 over 13), neither significant.
#:
#: IT DOES NOT MAKE A MARKET TRADEABLE. Five of the seven lost money on the
#: holdout under every rule tried, the whole grid included; that is a fact
#: about the market, and no ranking rescues it.
#:
#: Recorded in the seal, and `why` reads it back from there, so a coin-flip
#: null is always chosen by the rule its real winner was.
SELECT_RANKS = ("tstat", "growth")
SELECT_RANK = os.environ.get("EXNESS_SELECT_RANK", "tstat")
if SELECT_RANK not in SELECT_RANKS:
    raise SystemExit(f"EXNESS_SELECT_RANK={SELECT_RANK!r}; known: "
                     f"{', '.join(SELECT_RANKS)}")
...
def choose(symbol, family, results, bar=None, rank=None):
    """The winning cell of `family`, or `None`. `rank` is one of
    `SELECT_RANKS` and defaults to `SELECT_RANK`."""
    rank = SELECT_RANK if rank is None else rank
    ...
    if rank == "tstat":
        def key(item):
            return item[2].get("edge_vs_drift_t_stat") or 0.0, item[0]
    else:
        def key(item):
            return item[0]
    score, params, stat, robust, total = max(ranked, key=key)
...
MEMORY_RESERVE = float(os.environ.get("EXNESS_MEMORY_RESERVE_GB", "4")) * 1e9
...
def _init_worker(symbol, phase, specs, bar, only=None, window=None):
    ...
    install_fills(symbol, bar, quiet=True)
...
def select(symbol, workers, spread_bp=None, bar=None, only=None):
    ...
    payload = {
        ...
        "protocol": {
            ...
            # Absent from every older seal, all of which ranked on `growth`.
            ...
            "execution": {
                ...
                "mtm_peak": MTM_PEAK,
                "financing": "multiday: 17:00 New York rollover schedule; "
                             "current swap rates for every year",
...
def why(symbol, workers, spread_bp=None, requested=None, bar=None):
    ...
    if not families:
        print(f"{symbol} {label_bar(bar)}: no requested family runs here")
        return
    # THE NULL IS CHOSEN BY THE RULE ITS REAL WINNER WAS. A seal written before
    # `select_rank` existed ranked on `growth`, and a coin flip picked on the
    # t-statistic against it would be two selection rules compared, not a signal
    # against noise. With no seal on disk the current rule is the only one.
    rank = SELECT_RANK
    real = output_path(symbol, bar, requested)
    if os.path.exists(real):
        with open(real, encoding="utf-8") as handle:
            rank = json.load(handle)["protocol"].get("select_rank", "growth")
    print(f"{symbol} {label_bar(bar)}: null ranked on `{rank}`", flush=True)
    ...
    destination = os.path.join(
        RESULTS, f"{RESULT_PREFIX}_null_{symbol}_{label_bar(bar)}"
...
def main():
    parser = argparse.ArgumentParser(
        description="Family study for any CFD symbol at Exness or FundedNext, "
                    "at any timeframe.")
    parser.add_argument("--broker", choices=sorted(BROKERS), default=BROKER,
                        help="whose costs and fills price the study "
                             "(default: $CFD_BROKER or exness)")
    ...
    parser.add_argument("--bar-minutes", default="30",
                        help="bar size: 5, 15, 30, 60, 120, 240, or 1440 for "
