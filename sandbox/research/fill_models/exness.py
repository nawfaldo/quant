...
four-day sample that can establish an order of magnitude and nothing else.
    ...
    py -m sandbox.research.fill_models.exness selftest
    py -m sandbox.research.fill_models.exness coverage  <- read FIRST
    py -m sandbox.research.fill_models.exness precompute
    py -m sandbox.research.fill_models.exness book       <- the portfolio
    py -m sandbox.research.fill_models.exness cells      <- per sleeve, solo
    py -m sandbox.research.fill_models.exness spreads
    py -m sandbox.research.fill_models.exness sleeve
    py -m sandbox.research.fill_models.exness latency
    py -m sandbox.research.fill_models.exness slippage
"""
...
UTC = timezone.utc
OUT_PATH = os.path.join(os.path.dirname(__file__), "exness.json")
...
#: `cfd_families.SESSION` (verified equal for all six Dukascopy symbols) plus
...
def _writer():
    ...
    # fill_models -> research -> sandbox -> repository root.
...
def _point_size(symbol):
    ...
    what `cfd_families.fill_maps` does on every sweep. The terminal is
...
def minute_table(symbol):
    # One definition, in `cfd_families`, because `resolve` refuses a symbol
...
MAPS_PATH = os.path.join(os.path.dirname(__file__), "exness_maps.json")
...
def book(path=OUT_PATH, source=DEFAULT_FEED):
    ...
    Needs the maps on disk first::
        ...
        py -m sandbox.research.fill_models.exness precompute
        py -m sandbox.research.fill_models.exness book
    """
    from sandbox.research import exness_combined_strategies as cs
...
def cells(path=OUT_PATH, source=DEFAULT_FEED):
    ...
    portfolio.
        ...
        py -m sandbox.research.fill_models.exness cells
    """
    from sandbox.research import exness_combined_strategies as cs
    from sandbox.research import exness_latency as lat
...
def coverage(path=OUT_PATH, source=DEFAULT_FEED):
    ...
    is the fix.
        ...
        py -m sandbox.research.fill_models.exness coverage
    """
    import numpy as np
...
def tick_window_start(path=None, source=DEFAULT_FEED, symbols=None):
    ...
    if not spans:
        raise SystemExit(
            ...
            f"sandbox.research.fill_models.exness precompute "
