...
import { createEnvironmentRule, fetchBacktests, fetchEnvironmentRules, updateEnvironmentRule, deleteEnvironmentRule } from '../api'
import { type Environment } from '../types'
import BacktestResultSidebar from '../components/BacktestResultSidebar'
...
type RuleType = 'spread' | 'slippage'

const ruleTypes: RuleType[] = ['spread', 'slippage']
                ...
                <tbody className="divide-y divide-gray-900/40 bg-gray-950/10">
                  {rules.map((rule) => (
                    <tr key={rule.id} className="border-b border-[#212124] last:border-0 text-gray-300">
                      <td className="py-3.5 pl-6 font-semibold text-gray-200 capitalize">{rule.type}</td>
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  {missingRuleTypes.map((type) => (
                    <option key={type} value={type}>{type === 'spread' ? 'Spread' : 'Slippage'}</option>
                  ))}
                </select>
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-[10px] font-semibold tracking-widest uppercase text-gray-500">Value</label>
                <input type="text" inputMode="decimal" value={value} onChange={(event) => setValue(numericWithDot(event.target.value))} placeholder="0.0" className="w-full bg-black/20 border border-[#212124] text-sm text-gray-200 px-3 py-2 outline-none transition-colors placeholder:text-gray-600 select-text" />
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-[6px] bg-black/50">
          <div className="relative z-10 w-[450px] h-[240px] bg-[#121214] rounded-lg shadow-2xl flex flex-col overflow-hidden">
            ...
            <div className="bg-[#121214] px-4 pt-4 pb-4 flex flex-col gap-3 select-none">
              <div className="flex flex-col gap-2">
                <label className="text-[10px] font-semibold tracking-widest uppercase text-gray-500">Rule</label>
                <input type="text" value={editRuleType === 'spread' ? 'Spread' : 'Slippage'} disabled className="w-full bg-[#1A1A1E] border border-[#212124] text-sm text-gray-400 px-3 py-2 outline-none select-none cursor-not-allowed capitalize font-semibold" />
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-[10px] font-semibold tracking-widest uppercase text-gray-500">Value</label>
                <input type="text" inputMode="decimal" value={editValue} onChange={(event) => setEditValue(numericWithDot(event.target.value))} placeholder="0.0" className="w-full bg-black/20 border border-[#212124] text-sm text-gray-200 px-3 py-2 outline-none transition-colors placeholder:text-gray-600 select-text" />
              </div>
