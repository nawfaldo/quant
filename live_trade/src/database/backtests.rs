...
impl Database {
    ...
    pub async fn save_backtest(
        ...
        values.extend(REPORT_FIELDS.iter().map(|field| {
            if *field == "contribution" && report.get(*field).is_none() {
                "[]".to_owned().into()
            } else {
                json_db_value(&report[*field])
            }
        }));
...
fn backtest_json(row: QueryResult) -> Result<JsonValue, ApiError> {
    ...
    for (index, (name, kind)) in BACKTEST_COLUMNS.iter().enumerate() {
        let value = match kind {
            ...
            ColumnKind::Json => {
                let encoded: String = row.try_get_by_index(index)?;
                serde_json::from_str(&encoded).unwrap_or_else(|_| JsonValue::Array(Vec::new()))
            }
