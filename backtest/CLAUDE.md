...
Single-binary backtester. Point value is **instrument-dependent**, not symbol-dependent. `engine.instrument` (`Instrument` enum, set by the CLI before each run) selects the model: `.nq_mini` = E-mini Nasdaq-100 future (NQ) at **$20/point per contract**, `.nq_micro` = Micro E-mini (MNQ) at **$2/point per contract**, `.forex` = CFD at **$1/point per standard lot** (a 10-point move on 0.1 lot = $1). `lotMult()` in `engine.zig` switches on it. When the `nq` symbol is selected the CLI prompts "Instrument? 1. forex 2. nq mini 3. nq micro"; every other symbol is forced to `.forex` (no prompt). The futures instruments (`.nq_mini`/`.nq_micro`) trade in **whole contracts** — `sizeFor()` rounds the requested size to the nearest integer (min 1) when `engine.usesContracts()` (i.e. `instrument != .forex`), applied in `engine.backtest` when a position opens; forex keeps fractional lots. The CLI prompts and the report use "contract" terminology when `usesContracts()` and "lot" otherwise. `engine.instrumentName()` returns the label (`"forex"`/`"nq mini"`/`"nq micro"`) shown in the report and stored in the DB; `engine.pointValue()` exposes the per-point dollar value (= `lotMult()`) so the CLI can visualize transaction costs. Twelve modules under `src/`:
...
- `strategies/30m_buy.zig` — `OrbBuy` struct (example strategy). Imports `../engine.zig`, `../data.zig`, `../sizings/vol_target.zig`.
...
- `strategies/rth_vwap.zig` — `RthVwap` struct (port of Zarattini & Aziz's VWAP Trend Trading). 1-min bars (`{symbol}_1m`), RTH only (09:30–16:00 ET). Session VWAP = Σ(HLC₃ × vol)/Σ vol reset each day at the first RTH bar. Continuous long/short: enters on the first RTH bar (close vs VWAP), flips whenever a later close lands on the other side of the VWAP, flattens at 16:00 (emits `.close` at 15:59 → fills at 16:00 open). Same sizing model as OrbBuy (`sizing_mode` + `vol`). Shares the OrbBuy `/run` and `/tune` flows; tunable.
...
- `tune.zig` — `/tune` grid search, **parallelized across all CPU cores**. Used by OrbBuy and RthVwap (both consume `OrbGrid` / `OrbCombo` / `runOrb` / `printReportOrb`; the comptime strategy type is passed in by the CLI). Grid sweeps `base_contracts`; the chosen `sizing_mode` + `vol` params are carried on `OrbGrid` as fixed values (not swept) and applied to every combo. Report columns: Growth, MaxDD, Score, baseCon. (BuyHold has no params, so `/tune` rejects it.)
- `report.zig` — prints equity summary to stdout (`print`) and provides a no-print variant for the tuner (`summarize` → `db.Summary`). Fields printed: Instrument (`engine.instrumentName()`), Initial Balance, Final Balance, Net Growth %, Sharpe Ratio (annualized, Rf=0, daily returns bucketed by exit date via Welford online algorithm), Max Drawdown %, Avg Drawdown %, Max Intraday DD %, Avg Intraday DD %, Total Win $, Total Loss $, Max Daily Loss $ (worst single-day realized PnL, with losing-day count), Avg Daily Loss $ (mean over losing days; daily PnL bucketed by trade exit date — display-only, not persisted), Win Rate % (wins/total trades), Profit Factor (Total Win / |Total Loss|), Expectancy ($ per trade), Max Lose Streak, Avg Contracts (min/max), Total Days, Avg Weekly Gain $ (%), Avg Monthly Gain $ (%). Total days via Julian Day Number subtraction on first/last bar timestamps. **Drawdown is NOT computed here** — it is read from `result.max_drawdown` / `result.avg_drawdown` (all-time trailing) and `result.max_intraday_drawdown` / `result.avg_intraday_drawdown` (intraday trailing: peak resets each calendar day; max = deepest single-day dip, avg = mean of each day's worst dip, one sample per trading day), which the engine measures on the bar-by-bar mark-to-market equity curve (realized balance + open position's unrealized PnL at each bar's close). This captures dips *during* a held position (e.g. a buy-and-hold riding through a crash), which a trade-close-only equity curve would miss entirely. The intraday metrics are display-only (printed, not persisted to `backtests.db`). Marked at bar close, so intrabar extremes are not captured; requires the strategy to fetch the `close` column. `print` returns the full `db.Summary` (all 20 fields). `summarize` (tuner-only, never saved) returns the same struct with zeroes for fields it doesn't compute. All dollar amounts are formatted with thousands separators via `fmtDollars` (e.g. `$5,284.40`, negatives `-$1,234.56`).
- `db.zig` — SQLite persistence. `db.save(strategy_name, symbol, result, summary)` opens `backtests.db`, creates tables if needed, runs `runMigrations()` to add any missing columns to existing databases, then inserts one `backtests` row (all 20 report fields + metadata) and one `trades` row per trade (side, entry/exit ts, entry/exit price, PnL, contracts) in a single transaction. The `instrument` label (`engine.instrumentName()`) is also stored on the `backtests` row (column `instrument`, bound last so the other column indices are unchanged). `db.list(entries)` reads all rows from `backtests` and returns them as `BacktestEntry` values (id, strategy name, symbol, instrument). `db.delete(id)` deletes a backtest row and its associated trades in a transaction.
...
```
/run
  → "Strategy?  1. 30M_BUY  2. BUY_HOLD  3. RTH_VWAP"   ← 1 and 3 share the same flow
  → "Symbol?  1. NQ  2. GBPUSD  3. EURUSD"
  → "Instrument?  1. forex  2. nq mini  3. nq micro"   ← ONLY when symbol is NQ;
       other symbols are forced to forex and skip this question. Sets the point
       value (forex $1/pt·lot, nq mini $20/pt·contract, nq micro $2/pt·contract)
       and the contract-vs-lot terminology for the rest of the flow.

  ── if strategy 1 (30M_BUY) or 3 (RTH_VWAP) ──────────────────────────────
  → "Initial balance: $?"
  → "Lots? (e.g. 1) "
  → "Leverage? (1 = no extra leverage, 2 = 2× lots) "
  ...
  → "Slippage? (buy fill A->B, enter for <ref>-><buy>) "
      ...
      • Defaults and the price scale follow the SYMBOL, not the instrument:
        NQ → 0.4 spread / 0.2 slippage on a ~20000 ref price (indexScale());
        forex pairs → 0.0002 / 0.0001 on a ~1.10000 ref. NQ modeled as the
        forex instrument still uses NQ prices (scale = nqSelected()), only the
        $/lot point value comes from the instrument. The answered echo shows the
        resulting buy AND sell fills.

  ── shared ────────────────────────────────────────────────────────────────
  → report printed
  → "Save result? (y/n)"  ← single keypress, no Enter
      y → saved to backtests.db (all report fields + symbol stored)
      n → continue
```
...
```
  ── if strategy 1 (30M_BUY) or 3 (RTH_VWAP) ──────────────────────────────
  strategy → symbol → [instrument, if NQ] → balance → base contracts (list) → leverage → sizing → [4 vol params] → date range → spread → slippage → grid search
```
...
`/delete` lists all saved backtests (each row prefixed `#<id>` with strategy name, symbol, instrument) and prompts "Select id:". The entered value is matched against the actual `backtests.id` (linear scan over the listed entries), not a sequential position; an unknown id reports "No backtest with that id." The selected backtest and its trades are removed from `backtests.db` in a transaction.
...
## OrbBuy strategy (`strategies/30m_buy.zig`)

Opening-Range Breakout, long-only, 5-min bars. Key rules:

- **Opening range**: built from the **first five** 5m bars (timestamps 09:30–09:50).
  - `OR_high` = max **close** of those five bars — the breakout reference.
  - `OR_low` = lowest candle **body** (`min(open, close)`, no wicks) across all **six** bars (09:30–09:55, including the breakout trigger bar) — the stop level. Matches the red box drawn in the frontend.
- **Breakout trigger**: the single 09:55 bar. If `close(09:55) > OR_high`, emit `.long` → engine fills at the **open of the 10:00 bar** (1-bar delay).
- **Stop loss**: if any subsequent 5m close ≤ `OR_low`, emit `.close`.
- **Take profit (1:1 RR)**: `risk = entry − OR_low` (where `entry` is the actual 10:00 open). TP = `entry + risk`. Triggered when a 5m close ≥ TP price.
- **Time exit**: 13:55 bar → fills at 14:00 open (1-bar delay).
- One trade per day; no re-entry.
- **Position sizing** — `sizing_mode` (`sizing.Mode`) selects how each entry is sized; `vol` (`sizing.VolTarget`) holds the vol-target params/state. On every bar the strategy snapshots `base_contracts` (= CLI's `base_lots × leverage`, captured on the first `update`) and, when `sizing_mode == .vol_target`, calls `vol.onBar(close, day_changed)` to maintain σ̂. At the breakout it sets `self.contracts = base_contracts × (vol.multiplier() or 1.0)` just before emitting `.long`, because the engine reads `strat.contracts` at signal time (the iteration the signal is returned, `open_contracts = strat.contracts` in `engine.backtest`). With `.none` the multiplier is 1.0 (fixed lots). The vol math (EWMA of daily returns, `min(target/σ̂, max_mult)`, `min_days` warm-up, 1-day-ahead estimate) lives entirely in `sizings/vol_target.zig`; pair this with the engine's `warmup_days` buffer so σ̂ is primed before the evaluation window. Mode + params are chosen via the `/run` and `/tune` "Sizing?" prompts.
...
PnL uses `lotMult()` (symbol-dependent: $20/point for `nq`, $1/point otherwise) and the per-trade size `pos.contracts`, captured from `sizeFor(strat.contracts)` when the position opens (rounded to a whole contract for `nq`). Running `equity` is updated after each closed trade. Each `Trade` carries its own `contracts` (lot size), so PnL, the DB (`trades.contracts`, REAL), and the report all reflect the actual per-trade size. Fill prices are adjusted by `applyFillCost` (half-spread + fixed slippage) before PnL calculation.
...
- Ctrl+C in any `awaiting_*` prompt state cancels the prompt and returns to idle. The states cover the full `/run` flow (`awaiting_strategy`, `awaiting_symbol`, `awaiting_instrument`, `awaiting_balance`, `awaiting_base_contracts`, `awaiting_leverage`, `awaiting_sizing`, `awaiting_vol_target`/`_halflife`/`_maxmult`/`_mindays`, `awaiting_from`, `awaiting_spread`, `awaiting_slippage`), the BuyHold sub-flow (`awaiting_bh_balance`/`_lots`/`_from`/`_spread`/`_slippage`), the `/tune` equivalents (`awaiting_tune_strategy`, `awaiting_tune_symbol`, `awaiting_tune_balance`, …, `awaiting_tune_from`, `awaiting_tune_spread`, `awaiting_tune_slippage`), and `awaiting_delete`. When adding a prompt state, also add it to the Ctrl+C cancel switch in `cli.zig`.
