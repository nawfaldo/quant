...
```bash
zig build                          # produces zig-out/bin/questdb_importer
...
```bash
./questdb_importer data.csv --table my_table
```
...
```bash
./questdb_importer nq_1m.csv --table nq --aggregate
...
```bash
./questdb_importer nq-1m_bk_fixed.csv \
  --table nq \
  --delim ';' \
  --ts-col date \
  --ts-col2 time \
